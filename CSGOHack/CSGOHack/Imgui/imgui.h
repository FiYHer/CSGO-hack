...
enum ImGuiBackendFlags_
{
    ...
    // Back-end Renderer supports ImDrawCmd::VtxOffset. This enables output of large meshes (64K+ vertices) while still using 16-bits indices.
};
...
// 【使用此结构将大多数设置和输入/输出传达给 Dear ImGui】
// Access via ImGui::GetIO(). Read 'Programmer guide' section in .cpp file for general usage.
// 【通过 ImGui 访问::GetIO()。阅读 .cpp 文件中的"程序员指南"部分,以便使用】
//-----------------------------------------------------------------------------
...
struct ImGuiIO
{
    ...
    //【将索引映射到表示"本机"键盘状态的 KeyDown[512] 条目数组】
    ...
    //【设置为 false 以禁用闪烁光标,对于认为它分散注意力的用户】
    ...
    //【设置为 true,仅允许在从标题栏拖动单击时移动窗口。没有标题栏的 Windows 不受影响。】

    //------------------------------------------------------------------
    // Platform Functions 【平台功能】
    ...
    // Optional: Access OS clipboard 【访问操作系统剪贴板】
    ...
    //【您可以通过在渲染后调用 ImGui::getDrawData() 来获取 ImDrawData*】
    ...
    //------------------------------------------------------------------
    // Input - Fill before calling NewFrame() 【在调用 NewFrame()之前填充】
    //------------------------------------------------------------------
    ...
    //游戏板输入。通过 EndFrame() 清除回零。键盘键将自动映射,并由 NewFrame() 在此处写入。
    ...
    //------------------------------------------------------------------
    // Output - Retrieve after calling NewFrame() 【调用 NewFrame() 后检索】
    //------------------------------------------------------------------
    ...
    //【当io.WantCaptureMouse是true,imgui将使用鼠标输入,不将它们调度到您的主游戏/应用程序】
    ...
    //【当io.WantCaptureKeyboard是true,imgui将使用键盘输入,不将它们调度到您的主游戏/应用程序】
    bool        WantTextInput;
    ...
    //【当io.WantTextInput是true,你可以显示一个屏幕上的键盘】
    ...
    //【.ini 加载/保存处于活动状态时(io.IniFilename = NULL),这将设置为通知您的应用程序】
    bool        NavActive;
